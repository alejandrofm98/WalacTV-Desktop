...
-- Icon name -> PUA codepoint mapping (from uosc_icons.otf).
-- Uses PUA codepoints directly instead of ligature names because mpv <= 0.37
-- uses libass's simple shaper which does NOT process GSUB ligature substitutions.
-- PUA codepoints are resolved via the font's cmap table (simple shaper compatible).
local icons_map = require('lib/icons_map')

---Convert a Unicode codepoint to UTF-8 bytes (for codepoints <= U+FFFF, BMP).
---@param cp integer
---@return string
local function cp_to_utf8(cp)
	...
	else -- U+0800 to U+FFFF (all Material Icons PUA codepoints are in BMP)
		return string.char(
...
function ass_mt:icon(x, y, size, name, opts)
	...
	-- Resolve icon name to PUA codepoint for libass simple shaper compatibility
	local icon_char = icons_map[name] and cp_to_utf8(icons_map[name]) or name
	self:txt(x, y, opts.align or 5, icon_char, opts)
end
