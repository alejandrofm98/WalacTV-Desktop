...
defaults = {
	...
	top_bar_controls = 'no',
	top_bar_title = '${user-data/walactv/title}',
	top_bar_alt_title = '${user-data/walactv/subtitle}',
...
-- walactv: pcall wrapper so intl failure (nil script dir, etc.) doesn't abort uosc
local intl_ok, intl = pcall(require, 'lib/intl')
if not intl_ok then
	msg.warn('intl.lua failed to load (' .. tostring(intl) .. '), defaulting to English')
	intl = { t = function(text, a) return string.format(text or '', a or '') end }
end
t = intl.t
...
mp.add_forced_key_binding('esc', 'walactv-back', function()
	if Menu:is_open() then
		Menu:close()
	else
		mp.set_property('force-media-title', '__walactv_back__')
	end
end)
