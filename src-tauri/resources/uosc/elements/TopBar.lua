...
function TopBar:init()
	...
	self.back = {
		icon = 'arrow_back',
		command = function() mp.set_property('force-media-title', '__walactv_back__') end,
	}
...
function TopBar:decide_enabled()
	...
	self.enabled = self.enabled and (state.is_video or state.is_audio or options.top_bar_controls or options.top_bar_title ~= 'no' or state.has_playlist)
end
...
function TopBar:render()
	...
	local back_rect = {ax = ax, ay = ay, bx = ax + self.size, by = by}
	local back_hover = get_point_to_rectangle_proximity(cursor, back_rect) <= 0
	local back_opacity = back_hover and 1 or config.opacity.controls
	local back_size = self.size - margin
	local back_ax, back_ay = back_rect.ax + margin, back_rect.ay + margin

	cursor:zone('primary_down', back_rect, self.back.command)
	ass:rect(back_ax, back_ay, back_ax + back_size, back_ay + back_size, {
		color = back_hover and fg or bg,
		opacity = visibility * back_opacity,
		radius = state.radius,
	})
	ass:icon(back_ax + back_size / 2, back_ay + back_size / 2, back_size * 0.5, self.back.icon, {
		color = back_hover and bg or fg,
		border_color = back_hover and fg or bg,
		opacity = visibility,
