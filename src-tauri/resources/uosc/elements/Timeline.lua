...
function Timeline:update_dimensions()
	...
	local controls_space = round((options.controls_size + options.controls_margin * 2) * state.scale)
	self.ax = window_border_size
	self.ay = display.height - window_border_size - controls_space - self.size - self.top_border
	self.bx = display.width - window_border_size
	self.by = display.height - window_border_size - controls_space
...
function Timeline:render()
	...
	-- Keep a generous hit area while rendering a slim desktop scrubber.
	local bar_height = round((self.is_hovered or self.pressed) and 7 * state.scale or 4 * state.scale)
	local bax, bay, bbx, bby = self.ax, self.by - bar_height, self.bx, self.by
	local fax, fay, fbx, fby = 0, bay, 0, bby
	local fcy = bay - round(10 * state.scale)
	...
	-- Neutral rail and red watched progress, matching familiar TV streaming UI.
	ass:rect(bax, bay, bbx, bby, {
		color = '737373', opacity = 0.55 * visibility, radius = bar_height / 2,
	})

	-- Progress
	local function draw_progress()
		ass:rect(fax, fay, fbx, fby, {
			color = 'e50914', opacity = config.opacity.position * visibility, radius = bar_height / 2,
		})
	end
	...
	if visibility > 0 then
		ass:circle(fbx, fay + bar_height / 2, round(6 * state.scale), {
			color = 'e50914', opacity = visibility,
		})
	end
